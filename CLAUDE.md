...
1. `Sinks/AzureLogAnalytics/AzureLogAnalyticsSink.cs` implements `Serilog.Core.IBatchedLogEventSink`. Serilog's own `BatchingSink` owns all queuing, batching, retry, and shutdown flushing, so this class holds no threads or buffers. `EmitBatchAsync` wraps each `LogEvent` into `{ TimeGenerated, Event, Message }` and POSTs the array to `{Endpoint}/dataCollectionRules/{ImmutableId}/streams/{StreamName}?api-version=2023-01-01`. `OnEmptyBatchAsync` returns a completed task. Auth resolves once in the constructor to a single `TokenCredential`: `LoggerCredential.TokenCredential` when the caller supplies one, otherwise an `Azure.Identity` `ClientSecretCredential` built from tenant/client/secret. Each batch calls `GetTokenAsync` and sets `Authorization` on its own `HttpRequestMessage`, so the static shared `httpClient` carries no per-sink state. There is no local token cache, because `TokenCredential` implementations cache and refresh internally.
